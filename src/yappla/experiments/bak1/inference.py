...
class InferenceEngine:
    ...
    def query(self, goal: Fact, depth: int = 0, max_depth: int = 100) -> List[Substitution]:
        """
        Query the knowledge base for facts matching the goal.
    ...
    def _prove_all(self, goals: List[Fact], subst: Substitution, depth: int, max_depth: int) -> List[Substitution]:
    ...
    def _rename_rule_variables(self, rule: Rule) -> Rule:
        ...
        def rename_term(term):
            if isinstance(term, Variable):
                ...
                return Fact(term.predicate, new_args)
            else:
                return term
