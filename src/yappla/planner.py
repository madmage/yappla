...
class Planner:
    ...
    def set_domain(self, domain):
        # Compile action specs into primitive actions if needed.
        if domain.contains_action_specs():
            self._domain = domain.compile_action_specs()
        else:
            self._domain = domain
    ...
    def plan(self, initial_state: "State", goal=None):
        ...
        initial_state = State({**self._domain.get_initial_state(), **initial_state})
        ...
        while planning_iterations < self.max_iterations:
            ...
            if goal_reached:
                ...
                planner_result.plan = self._build_plan(goal_state=state, to_reach=to_reach)
                break
    ...
    def _build_plan(self, goal_state: "State", to_reach):
        """Reconstruct a plan from the explored predecessor relationships.

        Args:
            goal_state: The state that satisfies the goal.
            to_reach: List of tuples (prev_state, action_name, new_state) used to
                reconstruct the path.

        Returns:
            A `Plan` as a list of (state, action_name) pairs.
        """
        plan = Plan()
        # Compute the plan by starting from the goal backward to the initial state.
        state = goal_state
        plan.append((state, None))  # (goal state, no action)
        while state:
            for prev_state, action_name, new_state in to_reach:
                if new_state == state:
                    if prev_state:
                        # (current state, action to perform)
                        plan.append((prev_state, action_name))
                    state = prev_state
                    break  # expect only one matching in to_reach
        plan.reverse()
        return plan
